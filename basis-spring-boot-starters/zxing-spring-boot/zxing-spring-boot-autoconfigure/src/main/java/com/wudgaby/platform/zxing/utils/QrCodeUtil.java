package com.wudgaby.platform.zxing.utils;
...
import com.wudgaby.platform.zxing.QrImage;
...
public class QrCodeUtil {
    ...
    /**解析一张图多个二维码*/
    private static QRCodeMultiReader qrCodeMultiReader = new QRCodeMultiReader();
    ...
    public static Result[] decodeMultiQCodes(BufferedImage image) throws NotFoundException, IOException {
        ...
        if(ArrayUtils.isNotEmpty(results)){
            String text = Joiner.on(",").join(Arrays.stream(results).map(r -> r.getText()).collect(Collectors.toList())).trim();
