    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (user != null) {
            ...
            // 如果传递了租户编号，则进行比对租户编号，避免越权问题
                ...
                ServletUtil.write(response, JSONUtil.toJsonStr(ApiResult.failure("您无权访问该租户的数据")), MediaType.APPLICATION_JSON_VALUE);
                return;
            }
        }
        ...
        if (!isIgnoreUrl(request)) {
            ...
            if (tenantId == null) {
                ...
                ServletUtil.write(response, JSONUtil.toJsonStr(ApiResult.failure("请求的租户标识未传递，请进行排查")), MediaType.APPLICATION_JSON_VALUE);
                return;
            }
            // 3. 校验租户是合法，例如说被禁用、到期
            try {
                ...
                ServletUtil.write(response, JSONUtil.toJsonStr(ApiResult.failure(ex.getMessage())), MediaType.APPLICATION_JSON_VALUE);
                return;
            }
